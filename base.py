...
# Sliders for Indicator Weights
weights = {
    "gdp_weight": st.sidebar.slider("GDP Weight", 0.0, 1.0, 0.4),
    "bot_weight": st.sidebar.slider("Balance of Trade Weight", 0.0, 1.0, 0.2),
    "inflation_weight": st.sidebar.slider("Inflation Weight", 0.0, 1.0, 0.15),
    "interest_rate_weight": st.sidebar.slider("Interest Rate Weight", 0.0, 1.0, 0.15),
    "forex_reserves_weight": st.sidebar.slider("Forex Reserves Weight", 0.0, 1.0, 0.1)
}
...
if st.session_state.df.empty:
    ...
    if calculation_mode == "Basic Indicators":
        ...
        df["BRICS_Currency_Value"] = df.apply(
            lambda row: row["SBC_Weight"] * (
                weights["bot_weight"] + (row["BOT"] / row["GDP"]) * weights["bot_weight"] +
                weights["inflation_weight"] + (row["Inflation"] / 100) * weights["inflation_weight"] +
                weights["interest_rate_weight"] + (row["Interest Rate"] / 100) * weights["interest_rate_weight"] +
                weights["forex_reserves_weight"] + (row["Forex Reserves"] / 1e12) * weights["forex_reserves_weight"]
            ),
            axis=1
        )
    ...
    tab1, tab2, tab3 = st.tabs(["Data Overview", "Scenario Analysis & Forecasting", "Visualization"])
    ...
    with tab2:
        ...
        for i, country in enumerate(df["Country"]):
            with cols[i]:
                st.write(f"### {country}")
                exp = st.number_input(f"Exports ({country})", value=df.loc[df["Country"] == country, "Exports"].values[0], key=f"exp_{country}")
                imp = st.number_input(f"Imports ({country})", value=df.loc[df["Country"] == country, "Imports"].values[0], key=f"imp_{country}")
                gdp = st.number_input(f"GDP ({country})", value=df.loc[df["Country"] == country, "GDP"].values[0], key=f"gdp_{country}")
                df.loc[df["Country"] == country, ["Exports", "Imports", "GDP"]] = [exp, imp, gdp]
                st.session_state.df = df # Update session state with new values
                df["BOT"] = df["Exports"] - df["Imports"]
                df["SBC_Weight"] = df["GDP"] / df["GDP"].sum()
                df["Exchange_Rate_BC"] = df["BRICS_Currency_Value"].apply(lambda x: 1 / x if x > 0 else None)
